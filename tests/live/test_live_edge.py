...
async def _ephemeral_host(api: CloudflareAccessApi, http: httpx.AsyncClient) -> AsyncIterator[str]:
    ...
    # an uploaded Worker is not served on workers.dev until its subdomain route is enabled
...
async def test_live_foreign_hostname_is_refused(
    ...
    try:
        ...
        assert [e["code"] for e in refused.value.errors] == [12130], str(refused.value)
        assert "does not belong to zone" in str(refused.value)
    finally:
        await delete_run(api, RUN)
...
async def _save_clients(hass: HomeAssistant, entry: ConfigEntry, **sections: Any) -> str:
...
async def _register_script(hass: HomeAssistant, entry: ConfigEntry, name: str) -> dict[str, str]:
    ...
    return _shown(await _save_clients(hass, entry, scripts={"new_name": name}))
...
async def _lifecycle(
    ...
    # the origin checks below call /api/, which only the api component serves
    ...
    second = next(u for u in await hass.auth.async_get_users() if u.name == "second@example.com")
    ...
    print("== a bearer Access admitted reaches the origin with the assertion; the origin maps it")
    resp = await edge.get(
    ...
    # a service token carries no e-mail: Access puts its client id in `common_name`,
    # and the origin maps the assertion to the user with that username
    await add_user(hass, claims["common_name"], name="CI runner")
    origin = await hass_client_no_auth()
    # the origin honours the assertion only on the gated hostname and only behind
    # Cloudflare, which it recognises by the CF-Ray header
    headers = {
    ...
    resp = await origin.get("/api/", headers={k: v for k, v in headers.items() if k != "CF-Ray"})
    ...
    assert not await _save_clients(hass, entry, self_registering={"redirect_uris": [callback]})
    ...
    shown = _shown(
        await _save_clients(
            hass,
            entry,
            console_apps={"new_name": "Live client", "new_redirect_uris": [callback]},
        )
    )
    assert shown["client_id"] and shown["client_secret"], shown
    (console_app,) = entry.options["console_apps"].values()
    ...
    assert not await _save_clients(hass, entry, console_apps={"Live client": []})
    ...
    assert not await _save_clients(hass, entry, self_registering={"redirect_uris": []})
    ...
    print("== drift outside the integration (binding cookie on) breaks reuse; a reload repairs it")
    gate = await api.get_app(gate_id)
    assert gate
    # send back only what a dashboard edit would: the writable fields, with the tag
    keep = (
        "type",
        "name",
        "domain",
    ...
    assert not await _save_clients(hass, entry, scripts={"CI runner": ""})

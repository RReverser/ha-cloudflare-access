...
def _console_app(entry: MockConfigEntry) -> tuple[str, dict[str, Any]]:
    """The one console app in the options: its id and record."""
    ((cid, app),) = entry.options["console_apps"].items()
    return cid, app


def _script(entry: MockConfigEntry) -> tuple[str, dict[str, Any]]:
    """The one script in the options: its id and record."""
    ((sid, script),) = entry.options["scripts"].items()
    return sid, script


def _shown(markdown: str) -> dict[str, str]:
    """The values of one credentials block, as the page shows them."""
    found = dict(re.findall(r"(Client ID|Client secret|Authorization URL): `([^`]*)`", markdown))
    out = {
        "client_id": found.get("Client ID", ""),
        "client_secret": found.get("Client secret", ""),
        "authorization_url": found.get("Authorization URL", ""),
    }
    if match := re.search(r"valid until (\d{4}-\d{2}-\d{2})", markdown):
        out["expires_at"] = match[1]
    return out
...
async def test_a_script_client_gets_a_service_token_the_gate_accepts(
    ...
    script = _script(access.entry)[1]
    assert script["name"] == "Backup job"
    assert script["token_id"] == token["id"] and script["client_secret"] == shown["client_secret"]
    gate = cf.by_name(GATE)
    ...
    assert _script(access.entry)[1]["name"] == "Nightly backup"
...
async def test_a_lost_service_token_is_replaced_and_an_orphan_deleted(
    ...
    old_id = _script(access.entry)[1]["token_id"]
    ...
    await hass.async_block_till_done()
    script = _script(access.entry)[1]
    assert script["token_id"] != old_id and script["token_id"] in cf.service_tokens
    assert set(cf.service_tokens) == {script["token_id"], "theirs"}, (
        "the orphan of a script removed while Home Assistant was down goes; foreign tokens stay"
    )
    assert cf.by_name(GATE)["policies"][1]["include"] == [
        {"service_token": {"token_id": script["token_id"]}}
    ]
...
async def test_legacy_service_token_option_becomes_script_clients(
    ...
    script = _script(entry)[1]
    assert script["name"] == "Garage script"
    assert script["token_id"] == "tok-1" and script["client_secret"] is None
...
async def test_registered_client_gets_an_access_application_and_the_gate_accepts_it(
    ...
    app = _console_app(access.entry)[1]
    assert app["app_id"] == client["id"] and app["client_secret"] == shown["client_secret"]

    # the reload that follows the save makes the gate accept the app's tokens
    gate = cf.by_name(GATE)
...
async def test_self_registering_client_is_a_redirect_url_on_the_gate(
    ...
    assert [p["name"] for p in cf.by_name(GATE)["policies"]] == ["ha-access: allow"]
    assert access.entry.options["client_redirect_uris"] == [
...
async def test_clients_of_an_earlier_version_are_sorted_into_kinds(
    ...
    """Clients were subentries: 0.2.x had one type with the kind in its data and gave
    every client that logged people in an application, then briefly one type per kind.
    All of them move into the options. A self-registering app cannot use an
    application, so a 0.2.x client with published callbacks becomes a callback URL and
    its application is deleted as an orphan; any other keeps its application."""
    cf = fake_cloudflare
    cf.service_tokens["tok-n"] = {"id": "tok-n", "name": "x", "client_id": "n.access"}
    entry = make_entry(**{CONF_GATE_ENABLED: True}, minor_version=4)
    entry.add_to_hass(hass)
    hass.config_entries.async_add_subentry(
        entry,
        ConfigSubentry(
            data=MappingProxyType({"name": "Nightly", "token_id": "tok-n", "client_id": "n"}),
            subentry_type="script",  # the brief per-kind type
            title="Nightly",
            unique_id=None,
        ),
    )
    ...
    assert entry.subentries == {}, "nothing is a subentry any more"
    assert entry.options["client_redirect_uris"] == ["https://claude.ai/api/mcp/auth_callback"]
    (google,) = entry.options["console_apps"].values()
    assert google["name"] == "Google Home" and google["app_id"] == "old-Google Home"
    assert "kind" not in google
    (nightly,) = entry.options["scripts"].values()
    assert nightly["name"] == "Nightly" and nightly["token_id"] == "tok-n"
...
async def test_the_redirect_url_option_is_the_self_registering_list(
    hass: HomeAssistant, fake_cloudflare: FakeCloudflare, jwks_server: FakeJwks, alice: User
) -> None:
    entry = make_entry(
        **{
...
async def test_client_registration_survives_a_reload_and_a_lost_application(
    ...
    assert access.entry.runtime_data.client_apps == {_console_app(access.entry)[0]: client_id}
    ...
    app = _console_app(access.entry)[1]
    assert app["app_id"] == recreated["id"]
    assert app["client_secret"] == cf.secrets[recreated["id"]] != shown["client_secret"]
    gate = cf.by_name(GATE)

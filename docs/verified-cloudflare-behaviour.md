...
| A self-registered client's grant follows the allow policy: once the person's address leaves the gate's allow rule, the next refresh is refused (`invalid_grant`) within a minute, while the access token already issued keeps passing until it expires (15 minutes). So removing a person ends their agents' access at the next refresh, without any revoke; `revoke_user` and `revoke_tokens` add nothing to that. Only the callback list is not re-checked on refresh (the row above) | verified 25 Sep 2026 |
